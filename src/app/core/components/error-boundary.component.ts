        ...
        <div class="error-boundary-content">
          ...
          @if (showDetails()) {
            <div class="error-details">
              <h3>Error Details</h3>
              <pre class="error-stack">{{ getErrorInfo().stack || 'No stack trace available' }}</pre>
...
export class ErrorBoundaryComponent implements ErrorHandler {
  ...
  getErrorId(): string {
    ...
    return `ERR-${info.timestamp}-${(info.message.slice(0, 10).replace(/[^a-zA-Z0-9]/g, '')).toUpperCase()}`;
  }
  ...
  retry(): void {
    ...
    this.retryCount++;
    this.loggerService.info('Error boundary retry attempt', {
  ...
  goHome(): void {
    this.loggerService.info('Navigating home due to error', {
      errorId: this.getErrorId()
    }, 'error-boundary');
  ...
  reload(): void {
    this.loggerService.info('Reloading page due to error', {
      errorId: this.getErrorId()
    }, 'error-boundary');
  ...
  openHelp(): void {
    this.loggerService.info('Opening help documentation', {
      errorId: this.getErrorId()
    }, 'error-boundary');
  ...
  private getComponentName(error: any): string | undefined {
    // Try to extract component name from error stack or message
    if (error.stack) {
    ...
    if (error.message) {
      const componentMatch = error.message.match(/(\w+Component)/);
      if (componentMatch) {
        return componentMatch[1];
      }
    }
